    ...
    public static List<Runnable> shutdownAndAwaitTermination(final ExecutorService executorService,
        ...
        try {
            ...
            if (logger.isInfoEnabled()) {
                logger.info("[#CommonExecutors.safeShudown] [Shutdown " + executorService + " "
            ...
            if (!_terminateResult) {
                ...
                if (logger.isInfoEnabled()) {
                    logger.info("[#CommonExecutors.shutdownAndAwaitTermination] [ShutdwonNow " + executorService + " "
                            + (_terminateResult ? "Success" : "Fail") + "]");
                }
            }
        } catch (InterruptedException e) {
            if (logger.isErrorEnabled()) {
                logger.error("[#CommonExecutors.shutdownAndAwaitTermination] [ShutdwonNow " + executorService + " "
                        + "error]" + e.toString(), e);
            }
            _left = executorService.shutdownNow();
